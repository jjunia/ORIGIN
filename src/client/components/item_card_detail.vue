<template>
  <div class="item_card">
    ...
    <template v-if="userName == userId || isMyZoom == true">
      <div>
    ...
    <template v-else>
      <router-link :to="{ name:'OtherUser', params:{ detailUserId: detailUserId } }">
        <p class="item_card_name">
          <slot name="itemUserId"></slot>
          <span>
            <slot name="itemDate">服务器开小差了不给你数据</slot>
...
<script>
  ...
  export default {
    name: 'item_card',
    props: ['userId', 'playUrl', 'cardType', 'isMyZoom', 'contentId'],
    data() {
      return {
    ...
    methods: {
      ...
      deleteContent: function () {
        if (this.isMyZoom == false) {
          let params = {
            "key": "1",
            "userName": this.userName,
            "user_content_id": this.contentId
          }
          console.log(params);
